...
const PlayerEdit = () => {
  const {playerformvalues,onChangeHandlerPlayerForm,player,getPlayer,updatePlayer,errors,setErrors}= useContext(mainContext);
  let {id} = useParams();
  useEffect(()=>{
    getPlayer(id);
    setErrors({});
  ...
  return (
    <div className='m-2'>
      <form onSubmit={updatePlayer} className="max-w-md mx-auto p-4 bg-white shadow-md rounded-sm">
      ...
      <div className="my-4">
        <button className="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 text-white rounded-md">Update Player</button>
      </div>
